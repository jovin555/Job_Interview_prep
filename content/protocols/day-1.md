...
**Answer:** During IEC 60601 compliance testing for the Project Y maternal/infant care system at Company A, we encountered intermittent communication failures between the device and the printer unit over a custom serial protocol running on RS485. The printer would occasionally miss monitoring value packets, causing gaps in the printed strip chart during labor monitoring—a critical safety issue.

**Situation:** The device continuously transmitted uterine contraction data, maternal heart rate, and fetal heart rate to the printer over a multi-drop RS485 bus at 115200 baud. During regulatory testing, the printer would drop approximately 1 in every 500 packets without any error flags.
...
## Q5: How would you select between SPI and I2C for sensor communication in a battery-powered medical IoT device like the Project X?
**Answer:** For the Project X—which uses pressure sensors for flow capture, RGB LED feedback, and needs 7 days of continuous battery life—the choice between SPI and I2C depends on several factors:
